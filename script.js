document.getElementById('calculate').addEventListener('click', () => {
  ...
  Papa.parse(fileInput.files[0], {
    ...
    complete: (results) => {
      ...
      if (data.length === 0) {
        showError('Uploaded CSV file is empty!');
        return;
      }
      ...
      const isValidFile = requiredColumns.every(col => headers.includes(col.toLowerCase()));
...
function calculateMetrics(data, startDate, endDate) {
  ...
  data.forEach(row => {
    ...
    if (workDate >= startDate && workDate <= endDate) {
      ...
      const payout = parseFloat(row['payout'].replace('$', '').trim());
